...
inline void InitEngine(SDL_Surface* & screen, const char* WindowsTitle, unsigned int WindowsWidth, unsigned int WindowsHeight)
{
	atexit(SDL_Quit);
	...
	window = SDL_CreateWindow(WindowsTitle, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WindowsWidth, WindowsHeight, SDL_WINDOW_OPENGL);
...
inline void InitEngine(SDL_Surface* &screen, const char* WindowsTitle)
{
...
///////////////////////////////
inline SDL_Surface *ImgLoader(const char *file, bool bCKey, Uint8 r, Uint8 g, Uint8 b, Uint8 alpha)
{
	SDL_Surface *pic = IMG_Load(file); // From SDL_image.h , load the image to pic
									   // Log error message into stderr.txt if happened
	if (pic == NULL)
		fprintf(stderr, "Missing image %s : %s\n", file, IMG_GetError());
	if (bCKey) // Sets the color key (transparent pixel) in a blittable surface and RLE acceleration.
...
inline SDL_Surface *ImgLoader(const char *file, Uint8 r, Uint8 g, Uint8 b)
{
	return ImgLoader(file, 1, r, g, b, 0);
}

inline SDL_Surface* ImgLoader(const char* file, Uint8 r, Uint8 g, Uint8 b, Uint8 alpha)
{
	return ImgLoader(file, 1, r, g, b, alpha);
}

inline SDL_Surface *ImgLoader(const char *file)
{
...
inline Sound* LoadSound(const char* path)
{
	return Mix_LoadWAV(path);
}

inline Music* LoadMusic(const char* path)
{
